...
The Fabric config `visionPerspective` selects `HUMAN_CLIENT_CAMERA` (the default shared view) or `COMPANION_CAMERA` (a separate first-person render from the loaded remote AI_Companion entity). The companion mode is opt-in, samples at `visionCompanionCaptureIntervalMs=5000` by default, and requires the bot to be loaded by the human client's world. It does not change the human's displayed camera or need another account. On Minecraft 1.21.11 this second pass must use the complete game renderer; rendering the world alone produced a frame from the human camera while falsely labeling it with the companion's pose. The current implementation also waits for gameplay rather than capturing an open chat/menu. The source label is retained through the world model and conversation prompt. Visual claims remain approximate region/confidence hypotheses; only Mineflayer symbolic observations may supply exact positions, health, inventory, entities, or objective success. The corrected companion render passed a controlled human-versus-companion viewpoint comparison on this Fabric 1.21.11 installation; other GPUs and clients remain unverified.
...
When a question arrives just after chat closes, Node may wait asynchronously for the next gameplay frame (up to `VISION_FRESH_FRAME_WAIT_MS=6000` by default). This adds at most one bounded capture interval to the visual reply, while Mineflayer movement and survival keep running. If the bridge is offline or no clean frame arrives, the answer remains an honest "no new image" rather than using an old scene.
...
A controlled superflat-world check placed a three-block-wide green wall directly in front of the stopped companion while the human stood several blocks to the side. Minecraft's native `/spectate AI_Companion` showed the wall centered and large; the saved corrected `COMPANION_CAMERA` PNG matched, while the human's view showed only its distant edge. In dark rain Qwen sometimes mislabeled the Overworld as the Nether. The verified game dimension rejects that scene label, but the guard keeps independently visible, high-confidence color/shape descriptions without promoting a guessed block material. This does not make visual object recognition authoritative; the camera can turn or move before the next requested frame.
...
In `COMPANION_CAMERA` mode Fabric waits until no client screen is open, avoiding chat input and menus in the second render. Shared-camera mode retains its existing `GAMEPLAY`, `MENU`, `CHAT`, `INVENTORY`, or `OTHER_SCREEN` labels; world perception accepts only `GAMEPLAY`. Frames without UI metadata are conservatively treated as `OTHER_SCREEN`, so rebuild and install the updated Fabric jar. `[VISION] metrics` summarizes received/replaced frames, inference starts, accepted/stale observations, and preemption at most once per 60 seconds.
...
Visual configuration is fully listed in `.env.example`. Important defaults are `VISION_ENABLED=false`, loopback port `32145`, 2 MB / 1280×720 input bounds, a 6-second maximum wait for an explicit fresh frame, a 45-second background interval, and a 30-second observation TTL. The Fabric capture defaults to 960×540 at most once per second in shared-camera mode and once per five seconds in companion-camera mode; Qwen is not called for every captured frame.
...
For the separate AI viewpoint, set the Fabric `visionPerspective` to `COMPANION_CAMERA` and restart Minecraft. In a disposable superflat world, build a distinctive wall directly in front of the bot, say `停下`, and place the human player several blocks to one side so the two views visibly differ. Ask `你现在看到什么？` after closing chat. Compare the newest ignored debug PNG and JSON (`frame.perspective`, `frame.camera`) with Minecraft's native `/gamemode spectator` then `/spectate AI_Companion`: the wall must have the same central placement in both AI views, not the human view. Restore `/gamemode survival` afterward. If a frame contains a chat box or command suggestions, or mirrors the human view, do not trust its visual claims; rebuild/install the latest Fabric jar and retest. The bot need not identify the wall's exact Minecraft block type.
...
- Proactive speech is not a continuous monologue. A rejected repeated/unsupported line defers routine speech choices for two minutes; a line about both rain and night reserves both topics so the next line cannot simply rephrase half of it. Fresh unrelated world events can still prompt a new comment. Visual labels can be wrong even when the frame really comes from the companion camera, so uncertain visual details are not treated as server facts.
